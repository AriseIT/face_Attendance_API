...
def faceRcognition(rtsp, classfloder):
    rtsp1 = rtsp
    classfloder1 = classfloder
    # initializing MTCNN and InceptionResnetV1 
    mtcnn0 = MTCNN(image_size=240, margin=0, keep_all=False, min_face_size=40) # keep_all=False
    mtcnn = MTCNN(image_size=240, margin=0, keep_all=True, min_face_size=40) # keep_all=True
    resnet = InceptionResnetV1(pretrained='vggface2').eval() 

    dataset = datasets.ImageFolder(os.path.join(os.path.abspath(os.getcwd()),'StuData'))
    ...
    classpath =  os.path.join(os.path.abspath(os.getcwd()), 'Pstudents', classfloder, now.strftime("%d%m%Y"))
    try:
        print('Make folder', classpath)
                ...
                if prob > 0.90:
                    ...
                    try:
                        if name in attendance:
                            ...
                            saved_img = os.path.join(classpath, name+'_'+now.strftime("%d%m%Y_%H%M%S")+'.jpg')
                            ...
                            if min_dist > 0.90:
                                ...
                                path =  os.path.join(os.path.abspath(os.getcwd()), 'Astudents', classfloder, now.strftime("%d%m%Y"))
                                try:
                                    print('Make folder', path)
                                    os.makedirs(path)
                                except OSError as e:
                                    pass
                                saved_img = os.path.join(path, 'Unknown'+'_'+now.strftime("%d%m%Y_%H%M%S")+'.jpg')
                                cv2.imwrite(saved_img, crop)
    ...
    # img_name = os.path.join(os.path.abspath(os.getcwd()),'Detected',"{}.jpg".format(int(time.time())))
